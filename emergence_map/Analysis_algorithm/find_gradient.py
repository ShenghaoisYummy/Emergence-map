...
def get_shifted_fourier_chart(img):
    ...
    fourier_abs = np.abs(
        fourier_shifted)  # This array that we use is the absolute value of the shifted fourier transform
...
def get_row_values(fourier_abs, shiftrows, shiftcols):
    ...
    fourier_abs[indices[0]][indices[
        1]] = 0  # set the max value to 0 as this middle point is not useful (note that the index of this point is the middle of the 2D array)
...
def main_run():
    ...
    img = plt.imread('png_pict/1.png').astype(float)
